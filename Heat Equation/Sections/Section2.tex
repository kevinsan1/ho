...
\begin{enumerate}
	...
	% \end{equation*}
	\begin{align*}
		\frac{d}{dt} Q_{0,0}(t) &= \left( \frac{ Q_{1,0} +Q_{-1,0} +Q_{0,1} + Q_{0,-1} -4Q_{0,0} }{\Delta x \Delta y} \right) + S_{0,0}(t) \\
		&= \left( \frac{ Q_{1,0} +Q_{0,1} -2Q_{0,0} }{\Delta x \Delta y} \right) + S_{0,0}(t) \\
		\frac{d}{dt} Q_{m-1,n-1}(t) &= \left( \frac{ Q_{m-2,n-1} +Q_{m-1,n-2} -2Q_{m-1,n-1} }{\Delta x \Delta y} \right) + S_{m-1,n-1}(t) 
	\end{align*}
	Equations for boundaries not at a corner look like this: 
	\begin{align*}
		& \frac{d}{dt} Q_{i,0}(t) = \left( \frac{ Q_{i+1,0} +Q_{i-1,0} +Q_{i,1} + -3Q_{i,0} }{\Delta x \Delta y} \right) + S_{i,0}(t), & i &\neq 0,m-1 &\text{ and } &j=0 \\
		& \frac{d}{dt} Q_{i,n-1}(t) = \left( \frac{ Q_{i+1,n-1} +Q_{i-1,n-1} + Q_{i,n-2} -3Q_{i,n-1} }{\Delta x \Delta y} \right) + S_{i,n-1}(t), & i &\neq 0,m-1 &\text{ and } &j=n-1 \\
		& \frac{d}{dt} Q_{0,j}(t) = \left( \frac{ Q_{1,j} +Q_{0,j+1} + Q_{0,j-1} -3Q_{0,j} }{\Delta x \Delta y} \right) + S_{0,j}(t), & i &= 0 &\text{ and } &j\neq 0,n-1 \\
		& \frac{d}{dt} Q_{m-1,j}(t) = \left( \frac{Q_{m-2,j} +Q_{m-1,j+1} + Q_{m-1,j-1} -3Q_{m-1,j} }{\Delta x \Delta y} \right) + S_{m-1,j}(t), & i &= m-1 &\text{ and } &j\neq 0,n-1 \\
	\end{align*}
	\item Integrate with implicit Euler scheme 
	\begin{align*}
		& \frac{Q_{ij}^{n+1}(t)-Q_{ij}^{n}(t)}{\tau} = \Delta_5 Q_{ij}^{n+1} + S_{ij}^n(t) \\
		& Q_{ij}^{n+1}(t) = (\mathbf{I}-\tau \mathbf{\Delta_5})^{-1} (Q^n+S^n) 
	\end{align*}
